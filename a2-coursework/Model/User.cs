...
internal class User {
    public User(int staffID, string username, bool active, PrivilegeLevel? privilegeLevel, string forename, string surname, DateTime? dateOfBirth, string email, string? phoneNumber, string? emergencyContactForename, string? emergencyContactSurname, string? emergencyContactPhoneNumber, string? address, string? position, string? department, Theme theme) {
        StaffID = staffID;
        Username = username;
        Active = active;
