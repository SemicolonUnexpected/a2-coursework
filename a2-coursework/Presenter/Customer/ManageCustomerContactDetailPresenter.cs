...
public class ManageCustomerContactDetailsPresenter : BasePresenter<IManageCustomerContactDetailsView>, INotifyingChildPresenter {
    public event EventHandler? DetailsChanged;

    public ManageCustomerContactDetailsPresenter(IManageCustomerContactDetailsView view) : base(view) {
    ...
    private void ValidateContactInformation() {
        _emailValid = Validators.IsValidEmail(_view.Email);
        _view.SetEmailBorderError(!_emailValid);

        _phoneNumberValid = Validators.IsValidPhoneNumber(_view.PhoneNumber);
