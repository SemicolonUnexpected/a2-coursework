...
public class SecuritySettingsPresenter : SettingsPresenter<ISecuritySettings> {
    ...
    protected override bool ValidateInputs() {
        ...
        if (!ValidatePassword()) {
            _view.PasswordError = "Password does not satify the requirements";
            return false;
        }

        // The user put the same old password in
        if (CryptographyManager.GetHash(_view.NewPassword, _staff.Salt) == _staff.HashedPassword) {
    ...
    protected override void PopulateDefaultValues() {
        ...
        _view.JobTitle = _staff.Department;
    }
